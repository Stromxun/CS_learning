...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    reals = [i for i in paragraphs if select(i)]
    if k+1 <= len(reals):
        return reals[k]
    else: return []
    # END PROBLEM 1
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    if typed_words == 0 and source_words == 0:
        return 100
    elif typed_words == 0:
        return 0.0
    ...
    for i in range(len(typed_words)):
        if typed_words[i] == source_words[i]:
            accuracy_source += 1
    return 100*(accuracy_source/len(typed_words))
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    ls_num = len(str(typed))
    speed = (ls_num/5)/(elapsed/60)
    return speed
    # END PROBLEM 4
