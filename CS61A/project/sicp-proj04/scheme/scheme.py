...
def eval_all(expressions, env):
    ...
    second = eval_all(expressions.rest, env)
    return second if second is not None else first# replace this with lines of your own code
    # END PROBLEM 7
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        k, v = formals, vals
        while k is not nil:
            child_frame.define(k.first, v.first)
            k, v = k.rest, v.rest
        return child_frame
        # END PROBLEM 10
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    k, pre = expressions, True
    while k is not nil:
        tmp = scheme_eval(k.first, env)
        if is_false_primitive(tmp):
            return False
        k, pre = k.rest, tmp
    return pre
    # END PROBLEM 12
