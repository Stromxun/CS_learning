...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def find(place, cnt):
            if place.bees and (self.min_range <= cnt <= self.max_range):
                return bee_selector(place.bees)
            return find(place.entrance, cnt + 1) if place.entrance is not beehive else None
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.place.bees and not self.chewing:
...
class ScubaThrower(ThrowerAnt):
    ...
    def reduce_health(self, amount):
        if not (type(self.place) is Water):
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        # BEGIN Problem Optional 3
        insects, distance, now_place = {}, 0, self.place
        while now_place is not beehive:
            if now_place.ant and now_place.ant is not self:
                insects.update({now_place.ant : distance})
            for bee in now_place.bees:
                insects.update({bee : distance})
            distance += 1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 3
        return (2 - (0.25 * distance + 0.0625 * self.insects_shot))
        # END Problem Optional 3
