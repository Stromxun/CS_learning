...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_c = make_city('place', lat, lon)
...
def sprout_leaves(t, leaves):
    ...
    # new_branch = []
    # for b in branches(t):
    #     new_branch += [sprout_leaves(b, leaves)]
...
def coords(fn, seq, lower, upper):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [list([num, fn(num)]) for num in seq if fn(num) >= lower and fn(num) <= upper]
...
def add_trees(t1, t2):
    ...
    if t2:
        new_lable += label(t2)
    len_t1, len_t2, b1, b2= len(branches(t1)), len(branches(t2)), branches(t1), branches(t2)
    return tree(new_lable, [add_trees(b1[index] if index < len_t1 else [], b2[index] if index < len_t2 else []) for index in range(max(len_t1, len_t2))])
