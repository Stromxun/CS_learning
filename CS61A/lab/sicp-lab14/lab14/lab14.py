...
def prune_min(t):
    ...
    for i in range(len(t.branches)):
        if maxm < t.branches[i].label:
            maxm, delete = t.branches[i].label, i
    t.branches.pop(delete)
...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    def func(lst1, lst2, cnt, derive):
        if not derive:
            return 1 if 0 <= abs(sub(sum(lst1), sum(lst2))) <= d else 0
        newd = list(derive)
        num = newd.pop(0)
        return func(list(lst1) + [num], list(lst2), cnt + 1, newd) + func(list(lst1), list(lst2) + [num], cnt + 1, newd)
    return func([], [], 0, list(s)) // 2
...
def filterl(lst, pred):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldr(lst, lambda z, x: Link(x, z) if pred(x) else z, Link.empty)
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldl(lst, lambda z, x: Link(z, x), Link.empty)
